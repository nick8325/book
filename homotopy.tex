...
\caption{Homotopy groups of spheres~\cite{wikipedia-groups}.\protect\index{homotopy!group!of sphere}
The $k^\protect\textrm{th}$ homotopy group $\pi_k$ of the $n$-dimensional sphere
$\Sn^n$ is isomorphic to the group listed in each entry, where $\Z$ is
the additive group of integers,\protect\index{integers} and $\Z_{m}$ is the cyclic group\protect\index{group!cyclic}\protect\index{cyclic group} of order~$m$.
}
